package net.robertogonzalez.examen.services
...
class ConexionSQL(val miContexto:Context,
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db?.execSQL(query3)
        val query4 = "CREATE TABLE lista(idLista INTEGER PRIMARY KEY AUTOINCREMENT,  nombre TEXT, idPokemon INTEGER, idTipo INTEGER, idHabilidad, estado INTEGER )"
        db?.execSQL(query4)
    }
    ...
    fun listarLista() : ArrayList<Lista> {
        ...
        try {
            ...
            if (cursor?.moveToFirst() == true) {
                do {
                    val id = cursor.getInt(0)
                    val nombreLista = cursor.getString(1)
                    val idLis = cursor.getInt(2)
                    val idTip= cursor.getInt(3)
                    val idHab = cursor.getInt(4)
                    val estado = cursor.getInt(5)
    ...
    // Actualizar por tipo de pokemon
    ...
    fun actualizarPokemon(poke: Pokemon){
        try{
            ...
            db.close()
            if (result==0){
                Toast.makeText(miContexto,"Pokemon no ha sido actualizado", Toast.LENGTH_SHORT).show()
            }else{
